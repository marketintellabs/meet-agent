"""Google Meet connector using Playwright headless browser."""
...
# JS injected into the page to capture meeting audio via AudioContext
AUDIO_CAPTURE_JS = """
() => {
    ...
    // Intercept all audio elements and route them through our capture node
    ...
    // Also capture existing audio/video elements
    ...
    // Process captured audio into PCM chunks
...
# JS to play audio back into the meeting
AUDIO_PLAYBACK_JS = """
(b64Data) => {
...
class GoogleMeetConnector(MeetingConnector):
    """Joins Google Meet via headless Chromium and streams audio bidirectionally."""
    ...
    async def join(self, meeting_url: str) -> None:
        ...
        # Dismiss cookie consent / sign-in prompts if present
        ...
        # Wait until actually in the meeting
    ...
    async def set_video_frame(self, frame_rgba: bytes, width: int, height: int) -> None:
        # v0.2 — inject video frame into virtual camera feed
        pass

    # -- Private helpers --

    async def _dismiss_dialogs(self) -> None:
        """Click through cookie / sign-in overlays if present."""
        for selector in [
    ...
    async def _set_name(self) -> None:
        """Enter display name in the pre-join screen."""
        try:
            ...
            logger.debug("Name input not found — may already be signed in")
    ...
    async def _click_join(self) -> None:
        ...
        for text in ["Ask to join", "Join now", "Join"]:
            try:
                ...
                continue
        raise RuntimeError("Could not find a join button on the Google Meet page")
